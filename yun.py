...
class PositionTracker:
    ...
    async def on_fill(self, orderId, side, qty, price, sl, tp):
        ...
        async with self.lock:
            for pos in list(self.positions.values()):
                ...
                if hit_sl or hit_tp:
                    ...
                    # 撤销 SL/TP 挂单
                    if pos.sl_id:
                        await mgr.cancel_order(pos.sl_id)
                    if pos.tp_id:
                        await mgr.cancel_order(pos.tp_id)
                    # 市价平仓
...
class OrderManager:
    ...
    async def cancel_order(self, oid):
        ...
        ts = int(time.time()*1000 + time_offset)
        qs = urllib.parse.urlencode({'symbol':Config.SYMBOL,'orderId':oid,'timestamp':ts,'recvWindow':Config.RECV_WINDOW})
        sig= hmac.new(Config.SECRET_KEY, qs.encode(), hashlib.sha256).hexdigest()
    ...
    async def place(self, side, otype, qty=None, price=None, stop=None,
        ...
        async with self.lock:
            ...
            # 如果限价单部分成交，触发 on_fill
            exec_qty = float(data.get('executedQty', 0))
            if otype=="LIMIT" and exec_qty>0:
                await pos_tracker.on_fill(data, side, exec_qty, float(data.get('price',0)), extra_params.get('sl'), extra_params.get('tp'))
...
class MainStrategy:
    ...
    async def check(self, price):
        ...
        if strong:
            ...
            sizes  = [per]*len(levels)
        else:
            if side=="BUY":
                levels = [-0.0055,-0.0155] if trend_ok else [-0.0075]
            else:
                levels = [0.0055,0.0155] if trend_ok else [0.0075]
            per    = total_size / len(levels)
            sizes  = [per]*len(levels)
