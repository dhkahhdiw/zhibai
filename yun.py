...
class OrderGuard:
    ...
    async def check(self, strat, fp, trend_direction):
        async with self.lock:
            ...
            now = time.time()
            cd = self.cooldown.get(strat, 0)
            # 冷却
            if now - st.get('ts',0) < cd:
                LOG.debug(f"[Guard] {strat} 冷却中 {cd-(now-st['ts']):.1f}s")
                return False
            # 趋势必须交替
            if st.get('trend_direction') == trend_direction:
                LOG.debug(f"[Guard] {strat} 同趋势禁止 {trend_direction}")
                return False
            # 指纹不可重复
            if st.get('fp') == fp:
                LOG.debug(f"[Guard] {strat} 指纹重复 {fp}")
                return False
            return True

    async def update(self, strat, fp, trend_direction):
        async with self.lock:
            self.states[strat] = {'fp':fp, 'trend_direction':trend_direction, 'ts':time.time()}
...
class OrderManager:
    ...
    async def safe_place(self, strat, side, otype, qty=None, price=None, stop=None, extra_params=None):
        ...
        trend_dir = 'LONG' if side=='BUY' else 'SHORT'
        if not await guard.check(strat, fp, trend_dir):
            LOG.debug(f"[Mgr] 拦截 {strat} {side} 趋势{trend_dir}")
            return
        LOG.info(f"[Mgr] 允许下单 {strat} {side} 趋势{trend_dir}")
        await self.place(side, otype, qty, price, stop, sig_key=fp, extra_params=extra_params or {})
        await guard.update(strat, fp, trend_dir)
...
class MainStrategy:
    ...
    async def check(self, price):
        ...
        strong_long, strong_short = bb1<0.2, bb1>0.8
        ...
        if strong_long or strong_short:
            ...
            if side=="BUY":
                if trend_up: levels, sizes = [-0.0055,-0.0155],[0.03*0.5]*2
                else:        levels, sizes = [-0.0075],[0.015]
            else:
                if trend_dn: levels, sizes = [0.0055,0.0155],[0.03*0.5]*2
                else:        levels, sizes = [0.0075],[0.015]
        self._last=now
...
class TripleTrendStrategy:
    ...
    async def check(self, price):
        ...
        if up_all and not self.round_active and price<ma7<ma25<ma99:
            self._last=now; self.round_active=True
            p0=price*0.995; sl=price*0.97; tp=price*1.02
            ...
            self._last=now; self.round_active=True
            p0=price*1.005; sl=price*1.03; tp=price*0.98
