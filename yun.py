...
try:
    ...
    logging.error("Failed to load ED25519 key: %s", e)
    raise
...
class OrderGuard:
    ...
    async def check(self, strat, side):
        async with self.lock:
            s = self.states[strat]
            now = time.time()
            if now - s['last_ts'] < self._cd(strat):
                return False
            if strat=="main" and side==s['last_dir']:
                return False
    ...
    async def limit(self, strat, side, qty):
        lim = {"main":2.0,"macd":0.2,"rvgi":0.2,"triple":0.3}[strat]
        async with self.lock:
            p = self.pos[strat]
            return (p['long']+qty<=lim) if side=="BUY" else (p['short']+qty<=lim)

    async def update(self, strat, side, qty):
        async with self.lock:
            p = self.pos[strat]
            if side=="BUY":  p['long']  += qty
            else:            p['short'] += qty

    async def stamp(self, strat, side):
        async with self.lock:
            self.states[strat].update({
                'last_ts': time.time(),
                'last_dir': side,
                'waiting_cross': strat=="macd"
            })
...
class EnhancedOrderManager(OrderManager):
    ...
    async def safe_place(self, strat, side, otype, qty=None, price=None, stop=None, reduceOnly=False):
        ...
        if not await self.guard.check(strat, side):
            return
        if qty and not await self.guard.limit(strat, side, qty):
            return
        # rvgi 策略额外 MA 过滤
        if strat=="rvgi":
            df = data_mgr.klines["15m"]
            if len(df) >= 100:
                m7  = df["close"].rolling(7).mean().iat[-1]
                m25 = df["close"].rolling(25).mean().iat[-1]
                m99 = df["close"].rolling(99).mean().iat[-1]
...
class StrategyEngine:
    ...
    async def run(self):
        while True:
            # 等待任何 K 线更新
            await asyncio.gather(data_mgr.wait_update(), asyncio.sleep(0))
            # 校验实时价格未超时
...
class DynamicParameters:
    ...
    async def update(self):
        if time.time() - self.last < 60:
            return
        df = data_mgr.klines["15m"]
        if len(df) < 14 or latest_price is None:
            return
...
class MainStrategy:
    ...
    async def _run(self, price, st, bb3):
        ...
        otype = "STOP_MARKET" if side=="BUY" else "TAKE_PROFIT_MARKET"
        await mgr.safe_place("main", rev, otype, stop=slp)
