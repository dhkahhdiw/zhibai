...
async def detect_mode():
    ...
    is_hedge = (await r.json()).get('dualSidePosition', False)
...
class PositionTracker:
    ...
    async def on_fill(self, order_id, side, qty, price, sl, tp):
        ...
        # 同步下发主/备用账号止盈止损
        for manager, is_yz in ((mgr,False),(yz_mgr,True)):
        ...
        async with self.lock:
            if order_id in self.orders:
                ...
                if status in ('FILLED','CANCELED'):
                    pos.active=False
                    LOG.info(f"[PT] Closed cloid={pos.cloid} via remote")
    ...
    async def check_trigger(self, price):
        ...
        async with self.lock:
            for pos in list(self.positions.values()):
                ...
                triggered=False
                if pos.side=='BUY':
                    if price<=pos.sl_price+eps or price>=pos.tp_price-eps: triggered=True
                else:
                    if price>=pos.sl_price-eps or price<=pos.tp_price+eps: triggered=True
                if triggered:
...
# —— ListenKey & WebSocket 账户流 ——
async def get_listen_key():
    ts=int(time.time()*1000+time_offset)
    payload=f"timestamp={ts}"
    sig=base64.b64encode(ed_priv.sign(payload.encode())).decode()
    resp=await session.post(f"{Config.REST_BASE}/fapi/v1/listenKey",
                             headers={'X-MBX-APIKEY':Config.ED25519_API},params={'signature':sig,'timestamp':ts})
...
class OrderManager:
    ...
    async def safe_place(self,strat,side,otype,qty=None,price=None,stop=None,extra_params=None):
        ...
        LOG.info(f"[Mgr{'-YZ' if self.is_yz else ''}] {strat} → {side} {otype}")
    ...
    async def place(self,side,otype,qty,price,stop,sig_key,extra_params):
        ...
        LOG.debug(f"[Mgr{'-YZ' if self.is_yz else ''}] Sending {otype} {side} params: {params}")
        qs=urllib.parse.urlencode(sorted(params.items()))
        sig=hmac.new((self.SECRET_KEY if not self.is_yz else Config.YZ_SECRET_KEY),qs.encode(),hashlib.sha256).hexdigest()
        ...
        async with self.lock:
            ...
            except: LOG.error(f"[Mgr{'-YZ'}] NonJSON resp: {txt}"); return
            LOG.debug(f"[Mgr{'-YZ'}] Resp {otype} {side}: {data}")
            if r.status!=200:
                LOG.error(f"[Mgr{'-YZ'}] Order ERR {otype} {side}: {data}")
            else:
                LOG.info(f"[Mgr{'-YZ'}] Order OK {otype} {side}")
                if otype=="LIMIT" and float(data.get('executedQty',0))>0:
                    await pos_tracker.on_fill(data['orderId'], side, float(data['executedQty']), float(data.get('price',0)), extra_params.get('sl'), extra_params.get('tp'))
...
class MainStrategy:
    ...
    async def check(self,price):
        ...
        if strong:
            ...
            if side=="BUY":
                levels,sizes=([-0.0055,-0.0155],[0.015]) if price>df.st.iat[-1] else ([-0.0075],[0.015])
            else:
                levels,sizes=([0.0055,0.0155],[0.015]) if price<df.st.iat[-1] else ([0.0075],[0.015])
        self._last=now
        for lvl,sz in zip(levels,sizes):
            p0=price*(1+lvl if side=="BUY" else 1-lvl)
            sl, tp = price*(0.98 if side=="BUY" else 1.02), price*(1.01 if side=="BUY" else 0.99)
...
class MACDStrategy:
    ...
    async def check(self, price):
        ...
        if prev>0>curr and not self._in:
            sp=price*1.005
            await mgr.safe_place("macd","SELL","LIMIT",qty=0.07,price=sp,extra_params={'sl':sp*1.03,'tp':sp*0.97})
            self._in=True
        elif prev<0<curr and self._in:
            bp=price*0.995
            await mgr.safe_place("macd","BUY","LIMIT",qty=0.07,price=bp,extra_params={'sl':bp*0.97,'tp':bp*1.03})
            self._in=False
            ...
            async with websockets.connect(Config.WS_USER_BASE+listen_key) as ws:
                ...
                async for msg in ws:
                    ...
                    if data.get('e')=='ORDER_TRADE_UPDATE':
                        o=data['o']; await pos_tracker.on_order_update(o['i'], o['X'])
        except Exception as e:
