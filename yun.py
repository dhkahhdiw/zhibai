...
class Config:
    ...
    RECV_WINDOW       = 5000
    SYNC_INTERVAL     = 60
    ROTATION_COOLDOWN = 2200  # 同方向冷却 1 小时
...
class MACDStrategy:
    ...
    async def check(self, price):
        ...
        if prev>0>curr and not self._in:
            sp=price*1.005
            await mgr.safe_place("macd","SELL","LIMIT",qty=0.075,price=sp,extra_params={'sl':sp*1.03,'tp':sp*0.97})
            self._in=True
        elif prev<0<curr and self._in:
            bp=price*0.995
            await mgr.safe_place("macd","BUY","LIMIT",qty=0.075,price=bp,extra_params={'sl':bp*0.97,'tp':bp*1.03})
            self._in=False
