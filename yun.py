...
class DataManager:
    ...
    async def update_kline(self, tf, o,h,l,c,ts):
        async with self.lock:
            ...
            if tf=="15m":
                LOG.debug(f"15m Supertrend={df.st.iat[-1]:.2f} dir={'UP' if df.st_dir.iat[-1] else 'DOWN'} MACD diff={df.macd.iat[-1]:.4f}")
            if tf=="1h":
    ...
    async def check(self, strat, fp):
        async with self.lock:
            ...
            if fp == st.get('last_fp'):
                ...
                LOG.debug(f"{strat} 相同信号冷却中 {rem:.1f}s")
                return False
...
class OrderManager:
    ...
    async def place(self, side, otype, qty=None, price=None, stop=None, sig_key=None):
        ...
        if qty is not None:
            ...
            p["quantity"] = f"{q:.{qty_prec}f}"
        if price is not None:
            pr = quantize(price, price_step)
            p["price"] = f"{pr:.{price_prec}f}"
        if stop is not None:
            sp = quantize(stop, price_step)
            p["stopPrice"] = f"{sp:.{price_prec}f}"
        if otype == "LIMIT":
            p["timeInForce"] = "GTC"
...
class MainStrategy:
    ...
    async def check(self, price):
        ...
        self._last = now
        LOG.info(f"MAIN signal: trend={'UP' if up_trend else 'DOWN'}, strength={strength}")
        ...
        for lvl in levels:
            ...
            await mgr.safe_place("main", side, "LIMIT", qty=qty, price=price0,
...
class TripleTrendStrategy:
    ...
    async def check(self, price):
        ...
        if side:
            ...
            await mgr.safe_place("triple", side, "MARKET", qty=0.15, params={"state":self._state})
