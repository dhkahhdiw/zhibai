...
last_signal  = None    # 'UP' or 'DOWN' of last triggered order
macd_cycle   = None    # 'MACD_UP' / 'MACD_DOWN'
rvgi_cycle   = None    # 'RVGI_UP' / 'RVGI_DOWN'
triple_cycle = None    # 'TRIPLE_UP' / 'TRIPLE_DOWN'
...
async def order(side, otype, qty=None, price=None, stopPrice=None, reduceOnly=False):
    ...
    if otype == 'LIMIT':
        ...
        if notional < Config.MIN_NOTIONAL:
            logging.warning("Order notional %.2f < %.2f, skipped", notional, Config.MIN_NOTIONAL)
            return False
...
async def trend_watcher():
    ...
    while True:
        ...
        async with lock:
            df = klines['15m']
            if 'st' not in df or latest_price is None:
                continue
            stv = df['st'].iat[-1]
            dirc = df['st'].iat[-1] < latest_price
            trend = 'UP' if latest_price > stv else 'DOWN'
            if trend != last_trend:
                last_trend = trend
                last_signal = None   # 趋势切换时重置主策略下单标记
...
async def macd_strategy():
    ...
    while True:
        ...
        async with lock:
            df = klines['15m']
            if len(df) < 26 or 'macd' not in df:
                continue
            prev = float(df['macd'].iat[-2])
            cur  = float(df['macd'].iat[-1])
            osc  = abs(cur)
            # 银叉空
            if prev > 0 and cur < prev and 11 <= osc and osc < 20 and macd_cycle != 'DOWN':
                if last_signal != 'DOWN':
                    await order('SELL','MARKET',qty=0.017)
                    last_signal = 'DOWN'
                macd_cycle = 'DOWN'
            # 金叉多
            if prev < 0 and cur > prev and 11 <= osc and osc < 20 and macd_cycle != 'UP':
                if last_signal != 'UP':
...
async def rvgi_strategy():
    ...
    while True:
        ...
        async with lock:
            df = klines['15m']
            if len(df) < 10 or 'rvgi' not in df:
                continue
...
async def triple_st_strategy():
    ...
    while True:
        ...
        async with lock:
            ...
            if len(df) < 12:
                continue
            st1,d1 = supertrend(df,10,1)
            st2,d2 = supertrend(df,11,2)
