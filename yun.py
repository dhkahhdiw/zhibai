...
class Rotator(logging.Handler):
    def __init__(self,path,max_lines=1000):
        super().__init__(); self.path,path; self.max=max_lines
        open(path,'a').close()
    def emit(self,rec):
        msg=self.format(rec)
...
# —— 全局变量 ——
session:aiohttp.ClientSession; time_offset=0; latest_price=None; is_hedge_mode=False
...
class DataManager:
    def __init__(self):
        ...
        self.lock, self._evt = asyncio.Lock(), asyncio.Event()
        self.price, self.ptime = None,0

    async def load_historical(self):
        async with self.lock:
            for tf in self.klines:
                resp=await session.get(f"{Config.REST_BASE}/fapi/v1/klines",
    ...
    def _compute(self,tf):
        ...
        df["bb_up"],df["bb_dn"],df["bb_pct"] = m+2*s, m-2*s, (df.close-(m-2*s))/((m+2*s)-(m-2*s))
        if tf=="15m":
            hl2=(df.high+df.low)/2
            atr=df.high.rolling(10).max()-df.low.rolling(10).min()
            df["st"]=hl2-3*atr
            df["macd"]=MACD(df.close,12,26,9).macd_diff()
            rv=ROCIndicator(df.close-df.open,10).roc()
            df["rvgi"], df["rvsig"] = rv, rv.rolling(4).mean()

    async def get(self,tf,col):
        async with self.lock:
            df=self.klines[tf]
            return df[col].iat[-1] if col in df.columns and not df.empty else None
...
class PositionTracker:
    def __init__(self):
        self.long,self.short=0.0,0.0; self.lock=asyncio.Lock()
    async def sync(self):
        ts=int(time.time()*1000+time_offset)
        qs=urllib.parse.urlencode({'timestamp':ts,'recvWindow':Config.RECV_WINDOW})
        sig=hmac.new(Config.SECRET_KEY,qs.encode(),hashlib.sha256).hexdigest()
    ...
    async def place(self, side, otype, qty=None, price=None, stop=None, reduceOnly=False):
        ...
        async with self.lock:
            ...
            if r.status==200:
                ...
                txt=await r.text()
                LOG.error("Order %s %s ERROR %s (%.1fms)",otype,side,txt,latency)
...
async def user_ws():
    ...
    while True:
        try:
            ...
            LOG.error("User WS error: %s, retry 5s",e); await asyncio.sleep(5); retry+=1

# —— 配置热重载 & 维护 ——
async def watch_reload():
    async for ch in watchfiles.awatch('.'):
        for _,p in ch:
            if p.endswith(('.py','.env')):
                LOG.info("Reloading..."); load_dotenv(); await pos_tracker.sync()
