...
class Config:
    ...
    # 备用账号 YZ
    YZ_API_KEY          = os.getenv('YZ_BINANCE_API_KEY')
    YZ_SECRET_KEY       = os.getenv('YZ_BINANCE_SECRET_KEY').encode()
    YZ_ED25519_API      = os.getenv('YZ_ED25519_API_KEY')
    YZ_ED25519_KEY_PATH = os.getenv('YZ_ED25519_KEY_PATH')
...
# —— 加载 Ed25519 私钥 ——
ed_priv    = load_pem_private_key(open(Config.ED25519_KEY_PATH,'rb').read(), password=None)
yz_ed_priv = load_pem_private_key(open(Config.YZ_ED25519_KEY_PATH,'rb').read(), password=None)
LOG.info("Ed25519 keys loaded")
...
async def detect_mode():
    ...
    resp = await session.get(url, headers={'X-MBX-APIKEY':Config.API_KEY})
    is_hedge = (await resp.json()).get('dualSidePosition', False)
...
class PositionTracker:
    class Position:
        ...
        def __init__(self, cloid, side, qty, entry, sl, tp):
            ...
            self.entry_price, self.sl_price, self.tp_price = entry, sl, tp
            self.active = True
    ...
    async def on_fill(self, order_id, side, qty, price, sl, tp):
        async with self.lock:
            ...
            LOG.info(f"[PT] Open cloid={cloid} side={side} entry={price} SL={sl_q} TP={tp_q}")

        # 同步下发主/备用账号止盈止损
        for manager in (mgr, yz_mgr):
            close_side = 'SELL' if side=='BUY' else 'BUY'
            extra = {'closePosition':'true','workingType':'MARK_PRICE','priceProtect':'FALSE'}
            if is_hedge:
                extra.update({'positionSide': 'LONG' if side=='BUY' else 'SHORT','reduceOnly':'true'})
            await manager.place(close_side,'STOP_MARKET',stop=sl_q,extra_params=extra)
            await manager.place(close_side,'TAKE_PROFIT_MARKET',stop=tp_q,extra_params=extra)

    async def on_order_update(self, order_id, status):
        async with self.lock:
            if order_id not in self.orders: return
            cloid = self.orders[order_id]
            pos   = self.positions[cloid]
    ...
    async def check_trigger(self, price):
        ...
        async with self.lock:
            for pos in list(self.positions.values()):
                ...
                hit = False
                if pos.side=='BUY' and (price <= pos.sl_price+eps or price >= pos.tp_price-eps):
                    hit = True
                if pos.side=='SELL' and (price >= pos.sl_price-eps or price <= pos.tp_price+eps):
                    hit = True
                if hit:
    ...
    async def _local_close(self, pos, price):
        ...
        LOG.info(f"[PT] Local close MARKET {side}@{price} cloid={pos.cloid}")
        for manager in (mgr, yz_mgr):
            try:
                await manager.place(side,'MARKET',qty=pos.qty)
            except Exception as e:
                LOG.error(f"[PT] Local close failed on {manager.API_KEY}: {e}")
        pos.active=False
...
class OrderGuard:
    def __init__(self):
        ...
        self.lock     = asyncio.Lock()
        self.cooldown = {'main':Config.ROTATION_COOLDOWN,'macd':Config.ROTATION_COOLDOWN,'triple':Config.ROTATION_COOLDOWN}
    async def check(self, strat, fp, trend):
        async with self.lock:
            st= self.states[strat]; now=time.time()
            if trend==st['trend'] and now-st['ts']<self.cooldown[strat]: return False
            if st['fp']==fp: return False
            return True
...
# —— 下单管理 ——
class OrderManager:
    def __init__(self, api_key, secret_key, is_yz=False):
        self.lock       = asyncio.Lock()
    ...
    async def safe_place(self, strat, side, otype, qty=None, price=None, stop=None, extra_params=None):
        ...
        tag = "-YZ" if self.is_yz else ""
        LOG.info(f"[Mgr{tag}] {strat} → {side} {otype}")
        await self.place(side, otype, qty, price, stop, fp, extra_params or {})
        await guard.update(strat, fp, trend)

    async def place(self, side, otype, qty, price, stop, sig_key, extra_params):
        ...
        if otype in ("STOP_MARKET","TAKE_PROFIT_MARKET"):
            params.update({'closePosition':'true'}); params.pop("quantity",None)
            if is_hedge: params.update({'positionSide':'LONG' if side=='BUY' else 'SHORT','reduceOnly':'true'})
        if is_hedge and otype in ("LIMIT","MARKET"):
            params["positionSide"]= 'LONG' if side=='BUY' else 'SHORT'
        params.update(extra_params)
        tag="-YZ" if self.is_yz else ""
        LOG.debug(f"[Mgr{tag}] Sending {otype} {side} params: {params}")

        qs  = urllib.parse.urlencode(sorted(params.items()))
        key = self.SECRET_KEY
        sig = hmac.new(key,qs.encode(),hashlib.sha256).hexdigest()
        ...
        async with self.lock:
            ...
            except: LOG.error(f"[Mgr{tag}] Non-JSON resp: {txt}"); return
            if r.status!=200:
                LOG.error(f"[Mgr{tag}] Order ERR {otype} {side}: {data}")
            else:
                LOG.info(f"[Mgr{tag}] Order OK {otype} {side}")
                if otype=="LIMIT" and float(data.get('executedQty',0))>0:
                    await pos_tracker.on_fill(data['orderId'], side, float(data['executedQty']),
                                               float(data.get('price',0)), extra_params.get('sl'), extra_params.get('tp'))

# 主/备用下单管理
mgr    = OrderManager(Config.API_KEY,    Config.SECRET_KEY,    is_yz=False)
yz_mgr = OrderManager(Config.YZ_API_KEY, Config.YZ_SECRET_KEY, is_yz=True)
...
class MainStrategy:
    ...
    async def check(self, price):
        ...
        if strong:
            ...
            trend_ok=(side=="BUY" and price>st_line[-1] and st_dir[-1]) or (side=="SELL" and price<st_line[-1] and not st_dir[-1])
            qty=0.12 if trend_ok else 0.07
            levels=[0.0025,0.004,0.006,0.008,0.016]; sizes=[qty*0.2]*5
        else:
            st_line,st_dir=data_mgr.klines["15m"].st.iat[-1], None
            if side=="BUY":
                levels,sizes=([-0.0055,-0.0155],[0.015]) if price>st_line else ([-0.0075],[0.015])
            else:
                levels,sizes=([0.0055,0.0155],[0.015]) if price<st_line else ([0.0075],[0.015])
        ...
        for lvl,sz in zip(levels,sizes):
            ...
            await yz_mgr.safe_place("main",side,"LIMIT",      qty=sz, price=p0, extra_params={'sl':sl,'tp':tp})
        # 挂最后一个止损单
        sl=price*(0.98 if side=="BUY" else 1.02)
        await mgr.safe_place( "main", side, "STOP_MARKET", stop=sl)
        await yz_mgr.safe_place("main",side,"STOP_MARKET", stop=sl)
...
class MACDStrategy:
    ...
    async def check(self, price):
        ...
        if prev>0>curr and not self._in:
            sp=price*1.005
            for m in (mgr,yz_mgr):
                await m.safe_place("macd","SELL","LIMIT",qty=0.07,price=sp, extra_params={'sl':sp*1.03,'tp':sp*0.97})
            self._in=True
        elif prev<0<curr and self._in:
            bp=price*0.995
            for m in (mgr,yz_mgr):
                await m.safe_place("macd","BUY","LIMIT",qty=0.07,price=bp, extra_params={'sl':bp*1.03,'tp':bp*1.03})
            self._in=False
...
class TripleTrendStrategy:
    ...
    async def check(self, price):
        ...
        if up_all and not self.round_active:
            ...
            for m in (mgr,yz_mgr):
                await m.safe_place("triple","BUY","LIMIT",qty=0.05,price=p0,extra_params={'sl':sl,'tp':tp})
        elif dn_all and not self.round_active:
            self.round_active=True
            p0,sl,tp=price*1.005,price*1.03,price*0.98
            for m in (mgr,yz_mgr):
                await m.safe_place("triple","SELL","LIMIT",qty=0.05,price=p0,extra_params={'sl':sl,'tp':tp})
        elif flip_dn:
            for m in (mgr,yz_mgr): await m.safe_place("triple","SELL","MARKET")
            self.round_active=False
        elif flip_up:
            for m in (mgr,yz_mgr): await m.safe_place("triple","BUY","MARKET")
            self.round_active=False
            ...
            async with websockets.connect(Config.WS_USER_BASE + listen_key) as ws:
                ...
                async for msg in ws:
                    ...
                    if data.get('e')=='ORDER_TRADE_UPDATE':
                        o=data['o']
                        LOG.info(f"[WS USER] OrderUpdate id={o['i']} status={o['X']}")
