...
class OrderGuard:
    def __init__(self):
        ...
        self.lock  =asyncio.Lock()
    def _cd(self,s): return {"main":60,"macd":30,"rvgi":10,"triple":300}.get(s,0)
    ...
    async def run(self):
        while True:
            ...
            if price is None or time.time()-ts>60:
                logging.debug("Skipping strategy run: stale price")
                continue
...
class MainStrategy:
    ...
    async def check_signal(self,price):
        ...
        if bb3<=0 or bb3>=1:
            logging.info("MainStrategy triggered: bb3=%.3f st=%.3f price=%.3f", bb3, st, price)
...
class MACDStrategy:
    ...
    async def check_signal(self,price):
        ...
        if ml.iat[-2]<0<ml.iat[-1]:
            logging.info("MACD BUY cross")
            await self._enter("BUY")
        elif ml.iat[-2]>0>ml.iat[-1]:
            logging.info("MACD SELL cross")
