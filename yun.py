...
async def sync_time():
    ...
    srv_ms = js['serverTime']                      # 毫秒
    local_ms = int(time.time() * 1000)
    time_offset = srv_ms - local_ms                # 毫秒偏差
    ...
    async def update_kline(self, tf, rec):
        async with self.lock:
            ...
            if df.empty:
                ...
                # 按列安全更新最后一行
                for col, val in zip(['open','high','low','close'], [rec['o'],rec['h'],rec['l'],rec['c']]):
                    self.klines[tf].iat[-1, df.columns.get_loc(col)] = val
...
async def maintenance():
    ...
    while True:
        ...
        await pos_tracker.sync(); await order_mgr.sync(); await order_cleaner.check()
...
async def macd_strategy():
    ...
    while True:
        ...
        if prev>0 and cur<prev and osc>=11 and macd_cycle!='DOWN':
            macd_cycle='DOWN'; await order_mgr.place('SELL','MARKET',qty=0.017); await arbiter.register('macd','SELL')
        if prev<0 and cur>prev and osc>=11 and macd_cycle!='UP':
            macd_cycle='UP'; await order_mgr.place('BUY','MARKET',qty=0.017); await arbiter.register('macd','BUY')
