...
class OrderGuard:
    ...
    def _cooldown(self, strat):
        return {"main":60,"macd":30,"rvgi":150,"triple":300}.get(strat,0)
...
class EnhancedOrderManager(OrderManager):
    ...
    async def safe_place(self, strat, side, otype, qty=None, price=None, stop=None, reduceOnly=False):
        ...
        if qty and not await self.guard.limit(strat, side, qty):
            return
        if strat=="rvgi" and not await self._rvgi_ma(side):
            return
    ...
    async def _rvgi_ma(self, side):
        ...
        if side == "SELL":
            return latest_price > ma7 > ma25 > ma99
        else:
            return latest_price < ma7 < ma25 < ma99
...
class DynamicParameters:
    ...
    def get_order_levels(self):
        return [lvl * self.volatility for lvl in self.base_levels]

    def get_tp_levels(self):
        return [
            0.0102 * self.volatility,
            0.0123 * self.volatility,
            0.0150 * self.volatility * 1.2,
            0.0180 * self.volatility * 1.5,
            0.0220 * self.volatility * 2.0
        ]
...
class MainStrategy:
    ...
    async def process(self, price, st, bb3):
        ...
        qty = min(0.1 * strength * self.dyn.volatility, Config.MAX_POSITION * 0.3)
        # 分级挂单
...
class MACDStrategy:
    ...
    async def enter(self, side):
        for portion in (0.3, 0.5, 0.2):
            await mgr.safe_place("macd", side, "MARKET", 0.15 * portion)
...
class TripleTrendStrategy:
    ...
    async def check_signal(self, price):
        # 自动趋势反转平仓
        await order_guard.check("triple","")  # 触发内置检查
        df = data_mgr.klines["15m"]
        if len(df) < 12:
            return
        h,l,c = df["high"].values, df["low"].values, df["close"].values
