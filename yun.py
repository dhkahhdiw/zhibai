...
class Config:
    ...
    RECV_WINDOW        = 5000
    SYNC_INTERVAL      = 60
    ROTATION_COOLDOWN  = 1800  # 同向冷却时间（秒）
...
async def detect_mode():
    ...
    r   = await session.get(url, headers={'X-MBX-APIKEY':Config.BINANCE_API_KEY})
    data= await r.json()
...
class PositionTracker:
    ...
    async def on_fill(self, order_id, side, qty, price, sl, tp):
        ...
        # 止损单
        await mgr.place(close_side, 'STOP_MARKET', stop=pos.sl_price, extra_params=common)
        # 止盈单
        await mgr.place(close_side, 'TAKE_PROFIT_MARKET', stop=pos.tp_price, extra_params=common)
    ...
    async def check_trigger(self, price):
        ...
        async with self.lock:
            for pos in self.positions.values():
                ...
                trig = False
                if pos.side=='BUY':
                    if price <= pos.sl_price + eps: trig = True
                    elif price >= pos.tp_price - eps: trig = True
                else:
                    if price >= pos.sl_price - eps: trig = True
                    elif price <= pos.tp_price + eps: trig = True
                if trig:
    ...
    async def _local_close(self, pos):
        ...
        try:
            ...
            pos.active = False
            LOG.info(f"[PT] Local MARKET close cloid={pos.cloid}")
...
class DataManager:
        ...
        async with self.lock:
            for tf in self.tfs:
                ...
                df  = pd.DataFrame([{"open":float(x[1]),"high":float(x[2]),
                                     "low":float(x[3]),"close":float(x[4])} for x in arr])
                self.klines[tf]   = df
    ...
    def _compute(self, tf):
        ...
        if tf=="15m":
            ...
            df["st_line"], df["st_dir"] = numba_supertrend(df.high.values, df.low.values, df.close.values, 10, 3)
...
class OrderManager:
    ...
    async def place(self, side, otype, qty=None, price=None, stop=None, extra_params=None):
        ...
        # —— 必要：LIMIT 单必须带 timeInForce ——
        if otype=='LIMIT':
            params['timeInForce'] = 'GTC'

        # 条件单参数
        if otype in ('STOP_MARKET','TAKE_PROFIT_MARKET'):
            params.update({'closePosition':'true','workingType':'MARK_PRICE','priceProtect':'FALSE'})
            if is_hedge:
        ...
        async with self.lock:
            ...
            except: LOG.error(f"[Mgr] 非 JSON 返回: {text}"); return
...
class MainStrategy:
    ...
    async def check(self, price):
        ...
        if strong:
            ...
            levels=[0.0025,0.004,0.006,0.008,0.016]; sizes=[base*0.2]*5
        else:
            if side=='BUY':
                if price>df15.st_line.iat[-1]: levels,sizes=[-0.0055,-0.0155],[0.015,0.015]
                else: levels,sizes=[-0.0075],[0.015]
            else:
                if price<df15.st_line.iat[-1]: levels,sizes=[0.0055,0.0155],[0.015,0.015]
                else: levels,sizes=[0.0075],[0.015]
        ...
        for lvl,sz in zip(levels,sizes):
            ...
            await mgr.safe_place('main',side,'LIMIT',qty=sz,price=px,extra_params={'slPrice':sl,'tpPrice':tp})
...
class MACDStrategy:
    ...
    async def check(self, price):
        ...
        if prev>0>curr and not self.in_pos:
            px=price*1.005
            await mgr.safe_place('macd','SELL','LIMIT',qty=0.026,price=px,extra_params={'slPrice':px*1.03,'tpPrice':px*0.97})
            self.in_pos=True
        elif prev<0<curr and self.in_pos:
            px=price*0.995
            await mgr.safe_place('macd','BUY','LIMIT',qty=0.026,price=px,extra_params={'slPrice':px*0.97,'tpPrice':px*1.03})
            self.in_pos=False

class TripleTrendStrategy:
    def __init__(self): self.active=False; self.last=0
    async def check(self, price):
        now=time.time()
        ...
        if up_all and not self.active:
            ...
            await mgr.safe_place('triple','BUY','LIMIT',qty=0.015,price=px,extra_params={'slPrice':sl,'tpPrice':tp})
            self.active=True
        elif dn_all and not self.active:
            px=price*1.001; sl=price*1.03; tp=price*0.98
            await mgr.safe_place('triple','SELL','LIMIT',qty=0.015,price=px,extra_params={'slPrice':sl,'tpPrice':tp})
            self.active=True
        elif flip_dn:
    ...
    while True:
        try:
            ...
            async with websockets.connect(Config.WS_USER_BASE+listen_key, ping_interval=None) as ws:
                LOG.info("✔ Connected user stream"); retry=0
                async for msg in ws:
                    evt=json.loads(msg)
                    if evt.get('e')=='ORDER_TRADE_UPDATE':
                        o=evt['o']; await pos_tracker.on_order_update(o['i'], o['X'])
        except Exception as e:
...
async def engine():
    while True:
        ...
        for strat in strategies:
            ...
            except: LOG.exception(f"Strategy {strat.__class__.__name__} error")
