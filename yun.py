...
class MainStrategy:
    ...
    async def check(self, price):
        ...
        # 决策
        if up:
            side="BUY"; qty=0.12 if strong_long else 0.03; strength="STRONG" if strong_long else "WEAK"
        elif dn:
            side="SELL"; qty=0.12 if strong_short else 0.03; strength="STRONG" if strong_short else "WEAK"
        else: return
        self._last=now
