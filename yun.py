...
async def detect_mode():
    ...
    is_hedge_mode = (await (await session.get(url, headers={'X-MBX-APIKEY':Config.API_KEY})).json()) \
...
async def macd_strategy():
    ...
    while True:
        ...
        if len(df) < 27 or 'macd' not in df.columns:
            continue
        prev, cur = df['macd'].iat[-2], df['macd'].iat[-1]
        osc = abs(cur)
        if prev>0 and cur<prev and osc>=11 and macd_cycle!='DOWN':
            macd_cycle='DOWN'; await order_mgr.place('SELL','MARKET',qty=0.15)
        if prev<0 and cur>prev and osc>=11 and macd_cycle!='UP':
            macd_cycle='UP'; await order_mgr.place('BUY','MARKET',qty=0.15)
...
async def rvgi_strategy():
    ...
    while True:
        await asyncio.sleep(10)
        df = data_mgr.klines['15m']
        if len(df) < 11 or 'rvgi' not in df.columns:
            continue
...
async def triple_st_strategy():
    ...
    while True:
        ...
        if up and triple_cycle!='UP':
            triple_cycle='UP'; await order_mgr.place('BUY','MARKET',qty=0.15)
        if dn and triple_cycle!='DOWN':
            triple_cycle='DOWN'; await order_mgr.place('SELL','MARKET',qty=0.15)
