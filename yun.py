...
【三】挂单与止盈止损：
  - 3m布林带%B(20,2)触发下单：当%B ≤ 0触发买单，≥1触发卖单。
  - 根据信号与趋势关系确定下单规模：
      同趋势：强信号为0.12 ETH，弱信号为0.03 ETH；
      反趋势：强信号为0.07 ETH，弱信号为0.015 ETH。
  - 下单挂单采用固定价格偏移：强信号时使用5档，弱信号时使用2档；
  - 止盈挂单方案类似设置；
  - 止损：初始设置为买价×0.98（多单）/卖价×1.02（空单），并结合3m布林带带宽动态跟踪。
...
# 仓位上限控制：上升趋势：多仓0.49 ETH，空仓0.35 ETH；下降趋势：多仓0.35 ETH，空仓0.49 ETH
...
class ETHUSDCStrategy:
    ...
    async def get_hourly_strength(self, side: str) -> bool:
        ...
        percent_b = (close.iloc[-1] - lower.iloc[-1]) / denom if denom > 0 else 0.5
    ...
    async def analyze_order_signals_3m(self) -> Tuple[Signal, dict]:
        ...
        if percent_b <= 0:
            return Signal(True, 'BUY', {'trigger_price': float(close.iloc[-1])}), {}
        elif percent_b >= 1:
    ...
    async def close_position(self, side: str, qty: float, strategy: str = "normal") -> None:
        ...
        if current_price * qty < MIN_NOTIONAL:
            logger.error(f"[平仓] 市价单名义金额小于 {MIN_NOTIONAL} USDC，跳过")
            return
        params = {
            'symbol': SYMBOL,
            'side': side,
        ...
        try:
            data = await self.client._signed_request('POST', '/order', params)
            logger.info(f"[平仓] 市价平仓 {side}，数量: {qty}，返回: {data}")
        except Exception as e:
        ...
        while True:
            try:
                if self.last_trade_side == 'LONG':
                    if self.current_short > (0.75 * self.current_long):
                        excess = self.current_short - (0.75 * self.current_long)
                        logger.info(f"[封控] 空仓超限 {excess:.4f} ETH，平部分空仓")
                        await self.close_position(side='BUY', qty=excess, strategy="normal")
                elif self.last_trade_side == 'SHORT':
                    if self.current_long > (0.75 * self.current_short):
                        excess = self.current_long - (0.75 * self.current_short)
                        logger.info(f"[封控] 多仓超限 {excess:.4f} ETH，平部分多仓")
                        await self.close_position(side='SELL', qty=excess, strategy="normal")
            except Exception as e:
        ...
        while True:
            try:
                ...
                if self.last_trade_side == 'LONG' and latest < dynamic_stop:
                    ...
                    await self.close_position(side='SELL', qty=self.current_long, strategy="normal")
                elif self.last_trade_side == 'SHORT' and latest > dynamic_stop:
                    logger.info("[止损] 空单止损触发")
                    await self.close_position(side='BUY', qty=self.current_short, strategy="normal")
            except Exception as e:
            ...
            try:
                ...
                if self.prev_macd_off is not None:
                    if self.prev_macd_off < 0 and curr >= 0:
                        ...
                        await self.close_position(side='BUY', qty=self.current_short, strategy="macd")
                    elif self.prev_macd_off > 0 and curr <= 0:
                        logger.info("[MACD] 由正转负，触发市价平多")
                        await self.close_position(side='SELL', qty=self.current_long, strategy="macd")
