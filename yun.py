...
async def bracket(qty, entry, side):
    ...
    if tot > Config.MAX_POS_ETH:
        logging.warning("Exceed max pos=%.4f", tot)
        return
...
async def main_strategy():
    ...
    while True:
        async with lock:
            if any(klines[tf].empty for tf in ['3m','15m','1h']):
...
async def macd_strategy():
    ...
    while True:
        ...
        async with lock:
            ...
            if fired and prev<0 and cur>prev:
        ...
        try:
            async with websockets.connect(Config.WS_USER_URL) as ws:
                ...
                while True:
                    ...
                    params = {'apiKey':Config.ED25519_API,'timestamp':int(time.time()*1000),'recvWindow':Config.RECV_WINDOW}
                    sig = sign_ws(params)
                    rpc = {'id':req_id,'method':'order.status','params':{**params,'signature':sig}}
