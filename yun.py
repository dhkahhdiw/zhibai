...
async def order(side, otype, qty=None, price=None, stopPrice=None, reduceOnly=False):
    ...
    if otype=='LIMIT':
        ...
        if notional < Config.MIN_NOTIONAL_USD:
            logging.warning("Notional %.2f < %.2f, skipped", notional, Config.MIN_NOTIONAL_USD)
            return False
...
async def trend_watcher():
    ...
    while True:
        ...
        async with lock:
            df = klines['15m']
            if 'st' not in df.columns or latest_price is None: continue
            stv = df['st'].iat[-1]
    ...
    while True:
        ...
        async with lock:
            ...
            if last_signal is None and (bb3<=0 or bb3>=1):
                ...
                qty = (0.12 if strong else 0.03) if trend=='UP' else (0.07 if strong else 0.015)
                side, rev = ('BUY','SELL') if trend=='UP' else ('SELL','BUY')
                for off in levels:
                    po = p*(1+off if side=='BUY' else 1-off)
                    await order(side,'LIMIT',qty=qty,price=po)
                for off in tp_offs:
                    pt = p*(1+off if rev=='BUY' else 1-off)
                    await order(rev,'LIMIT',qty=qty*0.2,price=pt,reduceOnly=True)
                slp = p*(sl_up if trend=='UP' else sl_dn)
                ttype = 'STOP_MARKET' if trend=='UP' else 'TAKE_PROFIT_MARKET'
                await order(rev,ttype,stopPrice=slp)
                last_signal = trend
...
async def macd_strategy():
    ...
    while True:
        ...
        async with lock:
            df = klines['15m']
            if len(df)<26 or 'macd' not in df.columns: continue
...
async def rvgi_strategy():
    ...
    while True:
        ...
        async with lock:
            df = klines['15m']
            if len(df)<10 or 'rvgi' not in df.columns or 'rvsig' not in df.columns: continue
    ...
    while True:
        ...
        async with lock:
            ...
            if up and triple_cycle!='UP':
                if last_signal!='UP': await order('BUY','MARKET',qty=0.015); last_signal='UP'
                triple_cycle='UP'
            if dn and triple_cycle!='DOWN':
                if last_signal!='DOWN': await order('SELL','MARKET',qty=0.015); last_signal='DOWN'
                triple_cycle='DOWN'
