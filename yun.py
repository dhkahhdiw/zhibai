...
last_side  = None
lock       = asyncio.Lock()
# rate‐limit semaphore: max 5 messages/sec to order WS
...
async def init_order_ws():
    ...
    # ping loop to keep alive per docs
    async def ping_loop():
        while True:
            try:
                await order_ws.ping()
...
async def place_order_rpc(params: dict) -> dict:
    """
    Rate‐limited JSON‑RPC send with reconnect on policy violation.
    """
    global order_ws
    async with order_semaphore:
        # ensure no more than 5 msgs per second
        ...
        try:
            ...
            logging.warning("Order WS closed (%s), reconnecting...", e)
            await init_order_ws()
            # retry once
            await order_ws.send(json.dumps(req))
            resp = await order_ws.recv()
...
async def main_strategy():
    ...
    while True:
        ...
        async with lock:
            ...
            if bb1h is None or bb3m is None:
                await asyncio.sleep(0.5); continue
    ...
    while True:
        ...
        async with lock:
            ...
            if not triggered and prev>0 and macd<prev and macd>=11:
                await bracket('SELL',0.15, price); triggered=True
            if triggered and prev<0 and macd>prev and macd<=-11:
                await bracket('BUY',0.15, price); triggered=False
...
async def triple_st_strategy():
    ...
    while True:
        ...
        async with lock:
            ...
            p      = price
            st_seq = df['st']
            rising  = st_seq.iloc[-3] < st_seq.iloc[-2] < st_seq.iloc[-1] < p
            falling = st_seq.iloc[-3] > st_seq.iloc[-2] > st_seq.iloc[-1] > p
