...
class Config:
    ...
    MAX_POS         = 2.0
    HIST_LIMIT      = 1000
    ORDER_COOLDOWN  = 60        # 同一信号同方向最小间隔（秒）
...
async def load_symbol_filters():
    ...
    info = (await resp.json())['symbols']
    sym = next(s for s in info if s['symbol'] == Config.SYMBOL)
...
class OrderManager:
    ...
    async def place(self, side, otype, qty=None, price=None, stop=None, sig_key=None):
        ...
        async with self.lock:
            ...
            txt  = await resp.text()
            if resp.status == 200:
                LOG.info(f"Order OK {otype} {side} [{sig_key or ''}]")
            else:
                LOG.error(f"Order ERR {otype} {side} [{sig_key or ''}] {txt}")
...
class MainStrategy:
    ...
    async def check(self, price):
        ...
        # 分级挂限价
        for lvl in levels:
            px = price * (1 + (lvl if side=="BUY" else -lvl))
            sig = f"MAIN_{side}_{strength}_{lvl}"
            await mgr.place(side, "LIMIT", qty=qty, price=px, sig_key=sig)

        # 止盈
        for tp in tp_levels:
            sig = f"MAIN_TP_{side}_{strength}_{tp}"
            await mgr.place(side, "TAKE_PROFIT_MARKET", sig_key=sig)

        # 初始止损
        sl = price * (0.97 if side=="BUY" else 1.02)
        sig = f"MAIN_SL_{side}"
        await mgr.place(side, "STOP_MARKET", stop=sl, sig_key=sig)
...
class MACDStrategy:
    ...
    async def check(self, price):
        ...
        # 死叉做空
        if prev > 0 and cur < 0 and not self._in_round:
            offset = 0.005
            px     = price * (1 + offset)
            await mgr.place("SELL", "LIMIT", qty=0.15, price=px, sig_key="MACD_SELL")
            await mgr.place("SELL", "STOP_MARKET", stop=px * 1.03, sig_key="MACD_SELL_SL")
            await mgr.place("SELL", "TAKE_PROFIT_MARKET", sig_key="MACD_SELL_TP")
            self._in_round = True
        # 金叉做多
        elif prev < 0 and cur > 0 and self._in_round:
            offset = 0.005
            px     = price * (1 - offset)
            await mgr.place("BUY", "LIMIT", qty=0.15, price=px, sig_key="MACD_BUY")
            await mgr.place("BUY", "STOP_MARKET", stop=px * 0.97, sig_key="MACD_BUY_SL")
            await mgr.place("BUY", "TAKE_PROFIT_MARKET", sig_key="MACD_BUY_TP")
            self._in_round = False
