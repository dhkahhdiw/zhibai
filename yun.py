    ...
    async def update_kline(self, tf, rec):
        async with self.lock:
            ...
            if rec['t'] > self.last_ts[tf]:
                ...
                # 同周期内更新最后一行
                df.iloc[-1] = [rec['o'], rec['h'], rec['l'], rec['c']]
...
async def triple_st_strategy():
    ...
    while True:
        ...
        nonlocal_triple = locals().get('triple_cycle')
        if up and nonlocal_triple!='UP':
            locals()['triple_cycle']='UP'; await order_mgr.place('BUY','MARKET',qty=0.015)
        if dn and nonlocal_triple!='DOWN':
            locals()['triple_cycle']='DOWN'; await order_mgr.place('SELL','MARKET',qty=0.015)
        if nonlocal_triple=='UP' and not up:
            await order_mgr.place('SELL','MARKET',qty=0.015)
        if nonlocal_triple=='DOWN' and not dn:
