...
async def triple_st_strategy():
    ...
    while True:
        ...
        if up and triple_cycle!='UP':
            triple_cycle='UP'; await order_mgr.place('BUY','MARKET',qty=0.15); await arbiter.register('triple','BUY')
        if dn and triple_cycle!='DOWN':
            triple_cycle='DOWN'; await order_mgr.place('SELL','MARKET',qty=0.15); await arbiter.register('triple','SELL')
        # 反向平仓
