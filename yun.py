    ...
    async def update_kline(self, tf, o,h,l,c,ts):
        async with self.lock:
            ...
            if ts > self.last_ts[tf]:
                ...
                df.iloc[-1, df.columns.get_indexer(
...
class OrderGuard:
    ...
    async def update(self, strat, fp):
        async with self.lock:
            self.strategy_states[strat] = {'last_fp':fp, 'ts':time.time()}
...
class OrderManager:
    ...
    async def safe_place(self, strat, side, otype, qty=None, price=None, stop=None, params=None):
        ...
        if not await guard.check(strat,fp):
            return
        await self.place(side,otype,qty,price,stop,fp)
...
# —— 策略：Main ——
class MainStrategy:
    def __init__(self): self.interval=1; self._last=0
    async def check(self, price):
        ...
        strong_long = bb1<0.2; strong_short = bb1>0.8
        ...
        await mgr.safe_place("main", side, "STOP_MARKET",
                             stop=sl, params={"SL":True})
        await mgr.safe_place("main", side, "TAKE_PROFIT_MARKET",
                             params={"TP":True})
    ...
    async def check(self, price):
        ...
        if prev>0>cur and not self._in:
            ...
            await mgr.safe_place("macd", "SELL", "STOP_MARKET", stop=sp*1.03,
                                 params={"cross":"death_SL"})
            await mgr.safe_place("macd", "SELL", "TAKE_PROFIT_MARKET",
                                 params={"cross":"death_TP"})
            self._in = True
        ...
        # 金叉
            ...
            await mgr.safe_place("macd", "BUY", "STOP_MARKET", stop=bp*0.97,
                                 params={"cross":"golden_SL"})
            await mgr.safe_place("macd", "BUY", "TAKE_PROFIT_MARKET",
                                 params={"cross":"golden_TP"})
            self._in = False
