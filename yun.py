...
class OrderManager:
    ...
    async def place(self, side, otype, qty=None, price=None, stopPrice=None, reduceOnly=False):
        ...
        async with self.lock:
            ...
            logging.info("Order %s %s sent in %.1fms",otype,side,(time.time()-start)*1000)
            if res.get('code'):
                logging.error("Order ERR %s %s: %s",otype,side,res); return None
            oid = res['orderId']; self.active[oid]=res
            if otype=='LIMIT':
    ...
    async def cancel(self, oid):
        qs = urllib.parse.urlencode({
            'symbol':Config.SYMBOL,'orderId':oid,
        ...
        async with self.lock:
            ...
            if res.get('code'): logging.error("Cancel ERR %s: %s",oid,res)
            else: logging.info("Cancel OK %s",oid); self.active.pop(oid,None)
...
class SignalRouter:
    def __init__(self):
        self.last_trend=None; self.fired=False
...
async def maintenance():
    while True:
        ...
        await sync_time(); await order_mgr.sync()
...
def supertrend(df,period,mult):
    hl2=(df['high']+df['low'])/2; atr=df['high'].rolling(period).max()-df['low'].rolling(period).min()
...
async def triple_st_strategy():
    ...
    while True:
        ...
        dn=not(up or d2.iat[-1] or d3.iat[-1])
