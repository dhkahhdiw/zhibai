...
last_signal  = None     # 上次已触发方向：'UP' 或 'DOWN'
...
async def order(side, otype, qty=None, price=None, stopPrice=None, reduceOnly=False):
    """reduceOnly 只用于止盈限价单；止损用 STOP_MARKET/TAKE_PROFIT_MARKET."""
    ts = int(time.time()*1000 + time_offset)
    ...
    if otype=='LIMIT':
        ...
        if notional < Config.MIN_NOTIONAL_USD:
            logging.warning("Notional %.2f < %.2f skip", notional, Config.MIN_NOTIONAL_USD)
            return False
...
async def trend_watcher():
    ...
    while True:
        ...
        async with lock:
            df=klines['15m']
            if 'st' not in df or latest_price is None: continue
            stv=df['st'].iat[-1]
...
async def main_strategy():
    ...
    while True:
        ...
        async with lock:
            ...
            p=latest_price
            bb1=klines['1h']['bb_pct'].iat[-1]; bb3=klines['3m']['bb_pct'].iat[-1]
            stv=klines['15m']['st'].iat[-1]; trend='UP' if p>stv else 'DOWN'
            # 仅当 last_signal 为 None（本轮未下单）且 bb3 突破时触发
