...
class PositionTracker:
    ...
    async def on_fill(self, side, qty, entry, sl, tp):
        async with self.lock:
            ...
            # 下 STOP_MARKET 和 TAKE_PROFIT_MARKET，均捕获异常并重试一次
...
class MainStrategy(BaseStrategy):
    ...
    async def check(self, price):
        ...
        o = await exchange.create_order(SYMBOL, 'limit', side, qty, pr0, {'timeInForce': 'GTC'})
        if o.get('status') == 'closed': await pos_tracker.on_fill(side, qty, pr0, sl, tp)
        self._ts = time.time()
...
class MACDStrategy(BaseStrategy):
    ...
    async def check(self, price):
        ...
        if prev > 0 > curr and not self.in_pos:
            ...
            o = await exchange.create_order(SYMBOL, 'limit', 'sell', qty, pr0, {'timeInForce': 'GTC'})
        ...
        if prev < 0 < curr and self.in_pos:
            ...
            o = await exchange.create_order(SYMBOL, 'limit', 'buy', qty, pr0, {'timeInForce': 'GTC'})
            if o.get('status') == 'closed':
...
class TripleTrendStrategy(BaseStrategy):
    ...
    async def check(self, price):
        ...
        if dirc[-1] and not self.active:
            ...
            o = await exchange.create_order(SYMBOL, 'limit', 'buy', qty, pr0, {'timeInForce': 'GTC'})
            if o.get('status') == 'closed': await pos_tracker.on_fill('buy', qty, pr0, price * 0.97, price * 1.02)
            self.active = True
        if not dirc[-1] and not self.active:
            qty, pr0 = 0.017, price * 1.004
            LOG.info(f"▶️ Trend SELL@{pr0:.4f}")
            o = await exchange.create_order(SYMBOL, 'limit', 'sell', qty, pr0, {'timeInForce': 'GTC'})
            if o.get('status') == 'closed': await pos_tracker.on_fill('sell', qty, pr0, price * 1.03, price * 0.98)
            self.active = True
        self._ts = time.time()
    ...
    while True:
        ...
        for strat in strategies:
            try:
                ...
                LOG.error(f"{type(strat).__name__} ▶️ {e}")
