...
class PositionTracker:
    ...
    async def on_fill(self, order_id, side, qty, price, sl, tp):
        ...
        # 下发止损
        await mgr.place(close_side,'STOP_MARKET', stop=pos.sl_price, extra_params={
        ...
        # 下发止盈
        await mgr.place(close_side,'TAKE_PROFIT_MARKET', stop=pos.tp_price, extra_params={
            'closePosition':'true','workingType':'MARK_PRICE','priceProtect':'FALSE',
            **({'positionSide':'LONG' if side=='BUY' else 'SHORT','reduceOnly':'true'} if is_hedge else {})
        })
            ...
            for pos in list(self.positions.values()):
                ...
                if pos.side=='BUY':
                    if price <= pos.sl_price+eps:
                        LOG.info(f"[PT] SL@{price}<= {pos.sl_price}")
                        triggered = True
                    elif price >= pos.tp_price-eps:
                        LOG.info(f"[PT] TP@{price}>= {pos.tp_price}")
                        triggered = True
                else:
                    if price >= pos.sl_price-eps:
                        LOG.info(f"[PT] SL(short)@{price}>= {pos.sl_price}")
                        triggered = True
                    elif price <= pos.tp_price+eps:
                        LOG.info(f"[PT] TP(short)@{price}<= {pos.tp_price}")
                        triggered = True
                if triggered:
    ...
    async def _local_close(self, pos, price):
        ...
        LOG.info(f"[PT] Local MARKET {side}@{price} cloid={pos.cloid}")
        try:
...
async def get_listen_key():
    ...
    async with session.post(
        ...
        headers=headers, params={"signature":signature,"timestamp":ts}
    ) as resp:
            ...
            if r.status!=200:
                ...
                if sig_key and otype=="LIMIT":
                    ...
                    if sl is not None and tp is not None:
                        self.last_params[data['orderId']]={'sl':sl,'tp':tp}
...
class TripleTrendStrategy:
    ...
    async def check(self,price):
        ...
        if up_all and not self.round_active:
            ...
            await mgr.safe_place("triple","BUY","LIMIT",qty=0.03,price=p0,extra_params={'sl':sl,'tp':tp})
        elif dn_all and not self.round_active:
            self.round_active=True
            p0,sl,tp=price*1.005,price*1.03,price*0.98
            await mgr.safe_place("triple","SELL","LIMIT",qty=0.03,price=p0,extra_params={'sl':sl,'tp':tp})
        elif flip_dn:
            ...
            async with websockets.connect(Config.WS_USER_BASE + listen_key) as ws:
                ...
                async for msg in ws:
                    ...
                    if data.get('e')=='ORDER_TRADE_UPDATE':
                        o = data['o']
                        # === 新增：限价开仓成交时触发 on_fill ===
                        if o['o']=='LIMIT' and data.get('x')=='TRADE' and float(o['z'])>0:
                            params = mgr.last_params.get(o['i'],{})
                            await pos_tracker.on_fill(
                                o['i'], o['S'], float(o['z']), float(o['L']),
                                params.get('sl'), params.get('tp')
                            )
                        # 普通订单更新（FILLED/CANCELED）
