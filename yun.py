...
PRIVATE_KEY_PATH = os.getenv('ED25519_KEY_PATH')  # Ed25519 private key path
SYMBOL           = 'ETHUSDC'
...
RECV_WINDOW      = 5000  # Binance default recvWindow 币安更新文档.txt](file-service://file-UVewvyMfPwAbfn3RsNAkj1)
...
lock          = asyncio.Lock()
last_side     = None
session       = None  # aiohttp session, init in main()
...
def sign_ed25519(params: dict) -> str:
    # sort params, join key=value, sign with Ed25519, base64 and URL‑encode binanec api.txt](file-service://file-8dqRS7K67pjiL36CEoKdW7)
    ...
    while True:
        ...
        async with lock:
            ...
            if not trig and prev>0 and macd<prev and macd>=11:
                await bracket(0.15, latest_price, 'SELL')
                trig = True
            if trig and prev<0 and macd>prev and macd<=-11:
                await bracket(0.15, latest_price, 'BUY')
                trig = False
    ...
    while True:
        ...
        async with lock:
            ...
            if rv>sig and cnt_long*0.05 < 0.2:
                await bracket(0.05, latest_price, 'BUY')
                cnt_long += 1
            if rv<sig and cnt_short*0.05 < 0.2:
                await bracket(0.05, latest_price, 'SELL')
                cnt_short += 1
