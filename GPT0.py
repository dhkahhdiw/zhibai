...
# 尝试导入 aiodns 的 AsyncResolver，否则使用默认解析器
try:
...
# ==================== 日志配置 ====================
logging.basicConfig(
    level=logging.INFO,  # 需要调试时可改为 DEBUG
    ...
    async def fetch_klines(self) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            for i, candle in enumerate(data):
                # 请根据 USDC-M Futures 文档检查字段下标；本示例使用 [1,2,3,4,5,7]
    ...
    async def _execute_order(self, side: str, price: float):
        try:
            ...
            params = {
                'symbol': SYMBOL,
                'side': side,
                'type': 'STOP',  # 根据文档，如果应为 STOP_MARKET 请自行调整
